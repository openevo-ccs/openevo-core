...
- `oe:DesignPrinciple`, `oe:CurriculumDecision`, `oe:LearningDependency` — three of Addendum IV's originally-deferred general-meta-model types, now needed to make the real LPM repos' (`bio-core-k12`, `interdisciplinary-k12`) strand-level design choices traceable back to theory (see Addendum IV §4; the traceability chain was blocked pending these types). `oe:TheoryBundle` and the "executable theory / inference engine" ambition stay deferred — still too far from any concrete near-term need.
